    ...
    <PageTransition>
      <div style={{ minHeight: '100vh', display: 'flex', position: 'relative', zIndex: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 70% at 50% 50%, rgba(0,60,80,0.2) 0%, transparent 70%)' }} />
          <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 400 }}>
            ...
            <h1 style={{ fontFamily: 'Orbitron', fontSize: 'clamp(1.6rem,4vw,2.4rem)', fontWeight: 900, color: '#e8f4f0', lineHeight: 1.2, marginBottom: 16 }}>
              BUY DIRECT.<br /><span style={{ color: '#00d4ff', textShadow: '0 0 20px rgba(0,212,255,0.6)' }}>SAVE MORE.</span>
            </h1>
            ...
            <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'flex-start' }}>
              {['AI-verified quality scores', 'Full sensor transparency', 'Zero middlemen, zero commission'].map((f, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#7a9e8e', fontSize: '0.88rem' }}>
                  <span style={{ color: '#00d4ff', fontSize: '0.6rem' }}>✦</span> {f}
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
          style={{ width: '42%', minWidth: 360, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, background: 'rgba(2,6,12,0.6)', backdropFilter: 'blur(20px)', borderLeft: '1px solid rgba(0,212,255,0.1)' }}>
          <div style={{ width: '100%', maxWidth: 380 }}>
            <div style={{ fontFamily: 'Orbitron', color: '#00d4ff', fontSize: '0.7rem', letterSpacing: '0.2em', marginBottom: 8 }}>BUYER LOGIN</div>
            <h2 style={{ fontSize: '1.6rem', marginBottom: 4, color: '#e8f4f0' }}>Welcome Back</h2>
            <p style={{ color: '#3a5a4a', fontSize: '0.85rem', marginBottom: 32 }}>Sign in to browse verified produce</p>
            ...
            <form onSubmit={submit}>
              ...
              <button type="submit" disabled={loading}
                style={{ width: '100%', background: 'linear-gradient(135deg, #00d4ff, #0099cc)', border: 'none', color: '#020408', fontFamily: 'Space Grotesk', fontWeight: 700, padding: 14, borderRadius: 8, cursor: 'none', fontSize: '0.95rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 0 20px rgba(0,212,255,0.3)', transition: 'all 0.3s', marginTop: 8 }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 24, fontSize: '0.82rem', color: '#3a5a4a' }}>
              No account? <Link to="/register" style={{ color: '#00d4ff' }}>Register free</Link>
